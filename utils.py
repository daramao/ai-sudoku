...
def diagonal_units(rows, cols):
    """Diagonal product of rows and cols"""
    left_to_right_diag = []
    right_to_left_diag = []
    diagonal_units = [left_to_right_diag, right_to_left_diag]

    if len(rows) == len(cols):
        for each_row in rows:
            left_to_right_diag.append(each_row + cols[rows.index(each_row)])
            right_to_left_diag.append(each_row + str(len(rows) - rows.index(each_row)))
