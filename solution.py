...
unitlist = unitlist + diagonal_units(rows, cols)
...
def eliminate(values):
    ...
    """
    uniq_boxes = unique_values(values)
    for box in uniq_boxes:
        uniq_digit = values[box]
        for peer in peers[box]:
            values[peer] = values[peer].replace(uniq_digit,'')
            values = assign_value(values, peer, values[peer].replace(uniq_digit,''))
